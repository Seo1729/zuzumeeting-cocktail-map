...
만든 앱 → **앱 설정 → 앱 → 플랫폼 키** (예전 콘솔에서는 "앱 키"였습니다)
...
### 4-3. 카카오맵 서비스 켜기 — 앱을 만들었다고 켜져 있지 않습니다

**제품 설정 → 카카오맵 → 활성화 설정**을 **ON**으로 바꿉니다.

새로 만든 앱은 이 스위치가 **꺼져 있습니다.** 키가 맞고 도메인 등록이 끝나 있어도
꺼져 있으면 지도가 뜨지 않습니다. 실제로 이 단계에서 한 번 막혔습니다.

증상은 4-4(도메인 미등록)와 똑같이 "지도를 불러오지 못했습니다"라서 구별이 안 됩니다.
**둘을 구별하려면 카카오의 응답을 직접 봐야 합니다.** Git Bash에서:

```bash
curl.exe -i "https://dapi.kakao.com/v2/maps/sdk.js?appkey=본인_JavaScript_키&autoload=false"
```

| 응답 | 뜻 |
|---|---|
| `HTTP 200` + `window.kakao=...` 로 시작하는 JS | 키·활성화 모두 정상. 남은 문제는 도메인 등록(4-4) |
| `disabled OPEN_MAP_AND_LOCAL service` | **이 단계를 안 했습니다.** 카카오맵을 켜세요 |
| `NotExistApp` 등 | 키가 틀렸습니다 |

> PowerShell에서는 `curl`이 다른 명령의 별칭이라 실패합니다. **`curl.exe`** 로 부르세요.

### 4-4. 도메인 등록 — 빼먹으면 지도가 안 뜹니다
...
| `지도를 불러오지 못했습니다` (키 안내 문구 없이) | 키가 틀렸거나, **카카오맵이 꺼져 있거나(4-3)**, **도메인 등록이 안 됐습니다(4-4).** 셋을 화면으로는 구별할 수 없으니 **4-3의 `curl.exe` 명령으로 원인을 확인하세요** |
...
**방어는 4-4의 도메인 등록으로 합니다.** 등록된 도메인에서 온 요청만 지도가 뜨므로,
남이 키를 복사해가도 자기 사이트에서는 쓰지 못합니다.
그래서 4-4는 편의 설정이 아니라 **보안 설정**입니다.
