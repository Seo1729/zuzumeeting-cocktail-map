...
| 항목 | 어디까지 확인했나 | 왜 못 했나 |
|---|---|---|
| **서비스워커 / 오프라인** | `sw.js` 생성과 precache 항목을 정적으로 확인 | 개발 환경 브라우저가 서비스워커 등록을 차단 |
...
2. Cloudflare Pages 배포 (README 5절) → `pages.dev` 주소를 카카오 콘솔의
   **JavaScript SDK 도메인**에 추가 (README 4-4)
3. 부원 오픈채팅방에 링크 + "홈 화면에 추가" 안내
4. 신시가지 바 조사 — 상권 칩은 있는데 데이터가 0곳입니다
