...
export const MenuItem = z.object({
  ...
  */
  ...
  /*
    한 잔으로 파는 것인가, 병째 파는 것인가.

    isStraight와는 다른 축이다. isStraight는 "섞었나 안 섞었나"고 이건 "얼마만큼 파나"다.
    같은 맥캘란 12년이라도 잔은 18,000원, 병은 280,000원이라 서로 비교하면 안 된다.

    이 필드가 없을 때 실제로 사고가 났다. 오브가 띄어쓰기로 둘을 구분하고 있었는데
    ('맥캘란 12년'=잔술, '맥캘란12년'=보틀) 메뉴를 이름으로 묶는 과정에서 공백을 지우자
    둘이 한 상품으로 합쳐졌고, 가격 비교에 9,000원~160,000원처럼 나왔다.
    표기 습관에 의미를 싣지 않고 필드로 분리한다.
  */
  serving: z.enum(['glass', 'bottle']).default('glass'),
  /** 한 줄 설명. 운영진 코멘트. */
